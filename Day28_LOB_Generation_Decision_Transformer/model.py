...
ax2.set_title('Return-to-Go (RTG) Distribution')
...
# 4. Training Engine
# ==========================================
print("\n--- Phase 2: Autoregressive Trajectory Training ---")
...
for epoch in range(epochs):
    ...
    print(f"Epoch {epoch+1:02d}/{epochs} | Loss: {avg_loss:.4f} | Acc: {acc:.2f}% | Time: {time.time()-epoch_start:.2f}s")
...
# 5. Visualizations
# ==========================================
print("\n[SYSTEM] Rendering Training Metrics...")
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
ax1.plot(range(1, epochs+1), epoch_losses, marker='o', color='crimson')
ax1.set_title('DT Autoregressive Loss')
...
def simulate_trade_trajectory(target_return):
    ...
    with torch.no_grad():
        for t in range(5): 
            ...
            # 4. Synchronized Update (Advance all tensors to T+1)
            # We append the action we JUST took, so it acts as the "previous action" for the next tick.
