...
## LoRA: Full Mathematics and Intuition
...
The bottleneck forces the adaptation to capture only the most essential signal for the task. $A$ must learn which 8 directions in the 768-dimensional space are task-relevant. $B$ must learn how to broadcast those 8 signals back into the full representation space. The two matrices must cooperate: the compressor and broadcaster co-adapt during training.
...
The frozen backbone runs its full $O(L \cdot N^2 \cdot d)$ attention computation in the forward pass (no gradient). Gradients are computed only for the LoRA matrices:
...
The frozen BERT forward pass dominates; the LoRA addition is $O(N \cdot d \cdot r)$ per layer, negligible compared to the $O(N^2 \cdot d)$ attention. Measured inference latency: 122 to 131 ms per log line (including Python overhead from individual inference calls; batched inference would reduce this to single-digit milliseconds per sample).
...
Total parameters: 66,370,578. Trainable parameters: 6,160. Update ratio: 0.0093%.
...
- Initializing $B = 0$ ensures the model starts identical to the pre-trained baseline; fine-tuning is stable and the pre-trained knowledge is preserved from the first gradient step.
