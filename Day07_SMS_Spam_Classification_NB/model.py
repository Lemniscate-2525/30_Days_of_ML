...
# Dataset :
file_path = "/content/spam - Copy.csv"
...
sns.histplot(df[df['label'] == 0]['length'], color = 'red', label = 'Spam', kde = True)
sns.histplot(df[df['label'] == 1]['length'], color = 'blue', label = "Ham", kde = True)
plt.legend()
...
spam_text = " ".join(df[df['label'] == 0]['text'].tolist())
ham_text =  " ".join(df[df['label'] == 1]['text'].tolist())
...
prob_nbc = NBC.predict_proba(X_test)[:, 0] # Probabilities
...
prob_bbc = BBC.predict_proba(X_test)[:, 0] 
...
prob_lr = clf.predict_proba(X_test)[:, 0] 
...
def evaluate(name, y_true, y_pred, y_prob = None):
    ...
    if y_prob is not None:
        print("ROC AUC:", roc_auc_score(y_true == 0, y_prob))

evaluate("Multinomial NB", y_test, pred_nbc, prob_nbc)
evaluate("Bernoulli NB", y_test, pred_bbc, prob_bbc)
...
# ROC Comparison : 
fpr_nb, tpr_nb, _ = roc_curve(y_test == 0, prob_nbc)
fpr_lr, tpr_lr, _ = roc_curve(y_test == 0, prob_lr)
...
spam_wts = NBC.feature_log_prob_[0]
